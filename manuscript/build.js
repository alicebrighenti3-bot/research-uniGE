  ...
  sections: [
    {
      ...
      children: [
        ...
        p("Working title — draft manuscript v0.4", { italic: true, align: AlignmentType.CENTER, size: 22, spacingAfter: 100 }),
        ...
        p("The analysis plan below is specified in advance, as a tiered escalation, precisely so that the choice among tiers is dictated by what the identified literature actually supports rather than decided after seeing favourable results. All three tiers concern only the conditional, contingent component of this project (Section 2.6); they do not affect the scoping review and narrative framework, which proceed regardless."),

        h3("Tier 1 (default target if any quantitative synthesis proceeds): cancer as a pre-specified effect modifier within an APS-wide analysis"),
        p("A pairwise meta-analysis of the existing comparative evidence on anticoagulant strategy in high-risk/triple-positive thrombotic APS (RCTs and, analysed separately, observational cohorts/registries), framed around determinants of anticoagulant failure and bleeding, with active malignancy included as one of a small, fixed set of pre-specified effect modifiers alongside triple positivity and arterial-versus-venous index event (full list below). This tier is the realistic default because it requires only that individual studies report cancer status as a covariate or subgroup, not that any study was designed around an oncological population — a materially lower evidentiary bar than Tier 2."),

        h3("Tier 2 (conditional on data availability): a direct cancer-present versus cancer-absent comparison with formal interaction testing"),
        p("Attempted only if aggregate data separately reporting outcomes on VKA, DOAC, and/or LMWH within both a cancer and a non-cancer stratum are identified in at least two independent comparative sources. As noted in Section 4.1, the pivotal high-risk APS RCTs (TRAPS, Ordi-Ros et al., ASTRO-APS) are understood, on standard trial-design grounds, to have restricted enrolment to patients without active malignancy — a point to be explicitly confirmed against each trial's published exclusion criteria once full texts are reviewed, rather than assumed. If confirmed, these three RCTs contribute no data toward a treatment-by-cancer interaction test, and Tier 2 becomes dependent entirely on observational APS cohorts/registries that happen to report this specific cross-classification, which have not yet been identified. Where Tier 2 is achievable, the treatment × cancer-status interaction (and, where numbers allow, treatment × triple-positivity) will be tested directly as an interaction term, not inferred informally from within-stratum significance patterns, which do not themselves establish a subgroup difference."),

        h3("Tier 3 (aspirational, beyond the scope of this review): individual patient data meta-analysis"),
        p("An individual patient data (IPD) meta-analysis modelling treatment × cancer, treatment × triple positivity, and their interaction, adjusting for age, sex, arterial-versus-venous index event, prior recurrence, tumour type/stage, active oncological treatment, and platelet count, would represent the methodologically strongest possible answer to the research question — particularly because the cancer-plus-triple-positive subgroup is likely to be small and cross-classifications of interest are unlikely to be published in aggregate form. Assembling it would require direct collaboration with the investigators of the existing RCTs and of major APS registries (e.g., APS ACTION). This is listed in the Research Agenda (Section 4.3) as a future collaborative initiative rather than a deliverable of the present review."),

        h3("Statistical methods applying if Tier 1 or Tier 2 proceeds"),
        bullet("Outcome hierarchy: primary efficacy = recurrent thrombosis, analysed separately for venous and arterial events and never pooled together, since treatment effects and recurrence risk in high-risk APS are expected to differ by event type; primary safety = ISTH major bleeding. Secondary: clinically relevant non-major bleeding, all-cause mortality, treatment discontinuation/switching."),
        ...
        bullet("RCTs and observational studies (cohorts, registries) will be pooled separately as the primary analyses; a combined estimate will be presented only as a sensitivity analysis, given their differing susceptibility to confounding by indication. Risk of bias: RoB 2 for RCTs, ROBINS-I for observational studies. GRADE certainty rated separately for each primary and secondary outcome."),
        bullet("Pre-specified effect modifiers/subgroups (fixed and minimal, to avoid data-driven subgroup proliferation): triple-positive vs. non-triple-positive aPL profile; active malignancy present vs. absent; arterial vs. venous index event; specific DOAC agent (rivaroxaban vs. other DOACs) where reported; VKA intensity (INR 2.0–3.0 vs. higher-intensity regimens); RCT vs. observational design."),
        bullet("Network meta-analysis across VKA, DOAC, and LMWH is not planned by default: patients receiving LMWH in the available literature are drawn overwhelmingly from a cancer-associated-thrombosis indication systematically different from patients randomised to VKA/DOAC in APS trials, threatening the transitivity assumption a network model requires. A pairwise-only approach will be used unless a specific, separately justified transitivity assessment supports extending to a network model."),
        bullet("Meta-regression will be considered only as an exploratory, hypothesis-generating analysis, and only if at least 8–10 contributing datasets are available, given the recognised risk of ecological bias and overfitting with aggregate-data meta-regression on fewer studies; it will not substitute for the pre-specified subgroup/interaction analyses above."),
        ...
        bullet("Every pooled relative effect will be translated into an absolute-effect estimate (e.g., events avoided or incurred per 1,000 patients treated), using a representative baseline risk drawn from the included studies, to support clinical interpretability."),
...
Packer.toBuffer(doc).then((buffer) => {
  fs.writeFileSync("/home/user/research-uniGE/manuscript/APS_Cancer_Anticoagulation_Manuscript_v0.4.docx", buffer);
  console.log("done");
});
